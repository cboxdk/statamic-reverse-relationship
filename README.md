...
> **Fork notice:** This addon was originally created as part of [`tv2regionerne/statamic-reverse-relationship`](https://github.com/tv2regionerne/statamic-reverse-relationship) and is now maintained and developed independently under the Cbox namespace.   
> This version is a complete rewrite for Statamic 6, Laravel 12, Vue 3, and PHP 8.2+, and is not backwards-compatible with the original package.

A Statamic addon that lets you traverse relationships in reverse. If entry B points to entry A through a field, this addon lets A see all the B's that reference it — without storing anything on A.
...
That's it. No config files, no publishing, no migrations.
...
**With this addon:** Add the `reverse_relationship` fieldtype to your blog post blueprint, point it at the `comments` collection and the `post` field, and you're done. The addon figures out which comments reference each post at runtime.
...
This package **conflicts with** `tv2regionerne/statamic-reverse-relationship` — you cannot install both.

| | Original (tv2regionerne) | This fork (cboxdk) |
|---|---|---|
| Statamic | v4/v5 | v6 only |
| PHP | 8.1+ | 8.2+ |
| Frontend | Vue 2 Options API | Vue 3 Composition API |
| Template output | Eager `Collection` | Lazy query builder |
| Antlers tag | None | `{{ reverse_relationship }}` |
| Editable mode | No | Yes — attach/detach from CP |
| CP search | No | Yes — search related items |
| Test suite | None | 69 tests (Pest + PHPStan level 9) |

## Development

```bash
# Run everything (Pint + PHPStan + Pest)
composer check
...
Created by [Sylvester Damgaard](https://github.com/cboxdk). Originally built at [TV2 Regionerne](https://github.com/tv2regionerne/statamic-reverse-relationship), now maintained and actively developed at [Cbox](https://github.com/cboxdk).
